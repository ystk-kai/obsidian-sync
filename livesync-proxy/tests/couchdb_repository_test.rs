...
impl CouchDbRepository for InMemoryCouchDb {
    async fn get_document(&self, db_name: &str, doc_id: &str) -> Result<CouchDbDocument, DomainError> {
        ...
        Err(DomainError::CouchDbError(format!("Document {} not found in database {}", doc_id, db_name)))
    }
    ...
    async fn save_document(&self, db_name: &str, doc: CouchDbDocument) -> Result<CouchDbDocument, DomainError> {
        ...
        // データベースが存在しない場合は作成
        let db = databases.entry(db_name.to_string()).or_insert_with(HashMap::new);
    ...
    async fn delete_document(&self, db_name: &str, doc_id: &str, _rev: &str) -> Result<(), DomainError> {
        let mut databases = self.databases.lock().unwrap();
    ...
    async fn query_view(&self, db_name: &str, _design_doc: &str, _view_name: &str, _options: Value) 
        -> Result<Vec<CouchDbDocument>, DomainError> {
        let databases = self.databases.lock().unwrap();
    ...
    async fn ensure_database(&self, db_name: &str) -> Result<(), DomainError> {
        ...
        databases.entry(db_name.to_string()).or_insert_with(HashMap::new);
        
        Ok(())
    }
    
    async fn replicate(&self, source: &str, target: &str, _options: Value) -> Result<Value, DomainError> {
        let mut databases = self.databases.lock().unwrap();
        ...
        let docs_count = if let Some(db) = databases.get(source) {
            ...
            // ターゲットデータベースを取得または作成
            let target_db = databases.entry(target.to_string()).or_insert_with(HashMap::new);
            ...
            return Err(DomainError::CouchDbError(format!("Source database {} not found", source)));
        };
