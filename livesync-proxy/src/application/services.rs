...
impl LiveSyncService {
    ...
    async fn handle_sync(
        ...
        if let Some(_doc) = message.payload["document"].as_object() {
            let document: CouchDbDocument = serde_json::from_value(message.payload["document"].clone())
                .map_err(|e| DomainError::InvalidMessage(format!("Invalid document format: {}", e)))?;
