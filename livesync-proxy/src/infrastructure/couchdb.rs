...
impl CouchDbClient {
    ...
    pub async fn ping(&self) -> Result<(), DomainError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(DomainError::CouchDbError(format!("CouchDB ping failed with status: {}", status)));
        }
...
impl CouchDbRepository for CouchDbClient {
    async fn get_document(&self, db_name: &str, doc_id: &str) -> Result<CouchDbDocument, DomainError> {
        ...
        if response.status() == StatusCode::NOT_FOUND {
            return Err(DomainError::CouchDbError(format!("Document {} not found", doc_id)));
        }
        
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(DomainError::CouchDbError(format!("CouchDB error: {} - {}", status, error_text)));
        }
    ...
    async fn save_document(&self, db_name: &str, doc: CouchDbDocument) -> Result<CouchDbDocument, DomainError> {
        ...
        if !save_response.ok {
            return Err(DomainError::CouchDbError("Save operation not successful".to_string()));
        }
    ...
    async fn query_view(&self, db_name: &str, design_doc: &str, view_name: &str, options: Value) 
        ...
        let view_response = response
            .json::<ViewResponse>()
            .await
            .map_err(|e| DomainError::CouchDbError(format!("Failed to parse view response: {}", e)))?;
            
        let documents = view_response.rows
            .into_iter()
    ...
    async fn ensure_database(&self, db_name: &str) -> Result<(), DomainError> {
        ...
        if response.status() != StatusCode::NOT_FOUND {
            let status = response.status();
            return Err(DomainError::CouchDbError(format!("CouchDB error: {}", status)));
        }
        ...
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(DomainError::CouchDbError(format!("Failed to create database: {} - {}", status, error_text)));
        }
    ...
    async fn replicate(&self, source: &str, target: &str, options: Value) -> Result<Value, DomainError> {
        ...
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(DomainError::CouchDbError(format!("Replication failed: {} - {}", status, error_text)));
        }
        
        let result = response
            .json::<Value>()
            .await
            .map_err(|e| DomainError::CouchDbError(format!("Failed to parse replication response: {}", e)))?;
